...
/**
 * The Class BasicContext. This provide the basic context needed for the Chaos
 * Monkey to run. It will configure the Chaos Monkey based on a
 * simianarmy.properties file and chaos.properties. The properties file can be
 * overridden with -Dsimianarmy.properties=/path/to/my.properties
 */
public class GCloudChaosMonkeyContext extends GCloudSimianArmyContext implements
		ChaosMonkey.Context {
	 ...
	 */
	public GCloudChaosMonkeyContext() {
		...
		GCloudChaosCrawler chaosCrawler = new GCloudChaosCrawler(client());
		setChaosCrawler(tagKey.isEmpty() ? chaosCrawler
				: new FilteringChaosCrawler(chaosCrawler, new TagPredicate(
						tagKey, tagValue)));
		setChaosInstanceSelector(new BasicChaosInstanceSelector());
		setChaosEmailNotifier(new GCloudMonkeyEmailNotifier(null));
	}
	 ...
	 */
	protected void setChaosInstanceSelector(
